...
/* This function is called one time by a process using Mem_*
   ...
   so that you request base in units of the page size (see the
   manpages for getpagesize()). Note that you need to use this
   allocated base for your own data structures as well; that is,
   your infrastructure for tracking the mapping from addresses to
   base objects has to be placed in this region as well (it’s
   ...
   choosing a chunk of base for allocation. First-fit uses the first
...
/* This function is similar to the library function malloc().
   ...
   base region allocated by Mem_Init to satisfy this request. Free
   space is selected according to the policy specified by Mem_Init. */
void *Mem_Alloc(int size);

/* This function frees the base object that ptr falls within. Just
...
/* This function returns the fragmentation factor, which is defined as
   ...
   base.  If the fragmentation factor is 1, it means there’s no
   fragmentation (i.e., there’s only one free chunk); if it is close
   to 0, the entire free base is fragmented among small chunks. If
   there’s no more free base, this function returns 1. */
